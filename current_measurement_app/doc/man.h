 ...
 *  initialized by the relavant function to enable the bus communication. After initialization the measurements can be triggered and
 ...
 *  is overwritten by the settings on the sensor. The values PAC1720_device.DEV_name_opt, PAC1720_CH_config.CH_name_opt, PAC1720_device.DEV_sensor_address, 
 *  PAC1720_CH_config.CH_current_sense_resistor_value remain untouched. This is the simplest way to initialize the driver module since all settings are captured 
 *  from the sensor and only three mandatory settings are required as shown below.
 * 
 * @code
 ...
 * This function takes the configuration that is stored in the struct and initializes the sensor with it. Basically, the configuration can be made with default
 * values which are almost all equal to zero. To achive this, the struct is to be initialized accordingly. The setting of some values has to be 
